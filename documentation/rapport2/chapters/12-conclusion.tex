...
Ce projet de fin d'études a présenté la conception, l'implémentation et l'évaluation d'une architecture de sécurisation complète pour les flux de communication dans un écosystème IoT simulé. Les travaux ont été réalisés au sein de \TT{} en suivant la méthodologie Scrum, découpés en six sprints itératifs.
...
Le tableau~\ref{tab:objectifs} récapitule les sept objectifs définis dans l'introduction (chapitre~1) et leur degré de réalisation.
...
\begin{enumerate}
  ...
  \item \textbf{Pipeline de détection multi-niveaux} : Mise en place d'une chaîne de détection complète : IDS réseau (Suricata) $\rightarrow$ SIEM (Wazuh) $\rightarrow$ ML (IsolationForest + Random Forest), offrant une détection en profondeur.
...
\begin{itemize}
  ...
  \item \textbf{Dataset synthétique} : Le dataset utilisé pour le ML est partiellement synthétique. Les performances pourraient varier avec des données d'attaques réelles.

  \item \textbf{Scalabilité} : Les tests ont été limités à 50 dispositifs. Le passage à l'échelle (milliers de dispositifs) nécessiterait une validation supplémentaire.

  \item \textbf{Révocation} : Le mécanisme de révocation de certificats (CRL/OCSP) n'a pas été implémenté dans le prototype.

  \item \textbf{Détection en temps réel} : Le pipeline ML fonctionne en mode batch, pas en temps réel sur le flux MQTT.
\end{itemize}
...
\begin{itemize}
  \item Implémenter la révocation de certificats via OCSP stapling intégré à Vault.
  \item Déployer le pipeline ML en mode streaming avec Apache Kafka ou MQTT directement.
  \item Étendre le dataset avec des captures réseau réelles (pcap) pour valider les modèles.
  \item Ajouter le support TLS~1.3 0-RTT pour les reconnexions fréquentes des devices IoT.
\end{itemize}

\subsection{Moyen terme (6--12 mois)}

\begin{itemize}
  \item Migrer l'architecture vers Kubernetes (K3s) pour l'orchestration des conteneurs en production.
  \item Intégrer des techniques de Deep Learning (LSTM, Autoencoders) pour la détection de séquences d'attaques temporelles.
  \item Implémenter un \textit{Digital Twin} de l'infrastructure IoT pour les tests de pénétration automatisés.
  \item Développer un tableau de bord unifié (Grafana) consolidant les métriques de sécurité et de performance.
\end{itemize}

\subsection{Long terme ($>$ 12 mois)}

\begin{itemize}
  \item Évaluer le Post-Quantum TLS (ML-KEM / ML-DSA) pour anticiper la menace quantique sur la PKI~\cite{rfc9325}.
  \item Explorer la Federated Learning pour entraîner les modèles de détection directement sur les passerelles IoT, sans centraliser les données.
  \item Proposer un framework open source packagé \og IoT-Sec-Framework \fg{} réutilisable par d'autres organisations.
  \item Étendre l'approche à d'autres protocoles IoT : CoAP/DTLS, AMQP, LwM2M.
\end{itemize}
