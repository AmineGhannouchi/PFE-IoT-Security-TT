...
\midrule
\texttt{VAULT\_ADDR} & \texttt{http://192.168.30.10:8200} & Adresse de l'API Vault \\
\texttt{VAULT\_TOKEN} & \texttt{<root\_token>} & Token d'authentification \\
\texttt{MQTT\_BROKER} & \texttt{192.168.20.10} & IP du broker Mosquitto \\
\texttt{MQTT\_PORT\_TLS} & \texttt{8883} & Port MQTT sécurisé \\
\texttt{MQTT\_PORT\_TCP} & \texttt{1883} & Port MQTT non sécurisé \\
\texttt{CA\_CERT} & \texttt{/certs/ca-chain.crt} & Chaîne CA (root + intermediate) \\
\texttt{WAZUH\_MANAGER} & \texttt{192.168.40.10} & IP du manager Wazuh \\
\texttt{SURICATA\_EVE} & \texttt{/var/log/suricata/eve.json} & Fichier événements Suricata \\
\bottomrule
\end{tabular}
\end{table}
