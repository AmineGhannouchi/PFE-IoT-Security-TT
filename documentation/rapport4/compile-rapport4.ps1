...
$rapport3 = Split-Path -Parent $MyInvocation.MyCommand.Path
$projectRoot = Resolve-Path "$rapport3\..\.."

Write-Host '================================================' -ForegroundColor Cyan
Write-Host '  Compilation rapport3 -- PFE IoT Security TT'    -ForegroundColor Cyan
Write-Host '================================================' -ForegroundColor Cyan
Write-Host "Repertoire : $rapport3"
Write-Host ''
...
if ($Clean) {
    ...
    foreach ($ext in $extensions) {
        Get-ChildItem -Path $rapport3 -Filter $ext -ErrorAction SilentlyContinue |
            Remove-Item -Force
    }
...
if (-not $NoDiagrams) {
    ...
    $diagramsDir = Join-Path $rapport3 'diagrams'
...
# -- Etape 2 : Copier les figures depuis rapport2 (puis rapport1 en fallback) --
$figSrc = Join-Path $projectRoot 'documentation\rapport2\figures'
if (-not (Test-Path $figSrc)) {
    $figSrc = Join-Path $projectRoot 'documentation\rapport\figures'
}
$figDst = Join-Path $rapport3 'figures'
...
Push-Location $rapport3

$texFile = 'rapport.tex'
if (-not (Test-Path $texFile)) {
    Write-Host "[ERREUR] $texFile introuvable dans $rapport3" -ForegroundColor Red
    Pop-Location
    exit 1
}
